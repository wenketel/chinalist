...
(function(document, window) {
	...
	var list = [ [ /http:\/\/player\.youku\.com\//i, 'isAutoPlay=false', 'true', 'all', 'default' ],// youku外链。
	...
	function init(elem) {
		...
		case 'OBJECT': {
			...
			var allowFullScreen, allowNetworking, allowScriptAccess, flashVars;
			...
			for ( var i = params.length - 1, param_i; i >= 0; i--) {
				param_i = params[i];
				if (param_i && param_i.nodeName != 'PARAM')
					continue;

				if (allowFullScreen && allowNetworking && allowScriptAccess && flashVars && matched)
					break;
